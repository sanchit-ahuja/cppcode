...
bool checkCycle(unordered_map<char,unordered_set<char>>&parent,unordered_set<char>&uniqueVertices)
{
...
string getSmallestExpression(unordered_map<char, unordered_set<char>> &parent, char root)
{
    // cout<<root<<endl;
    ...
    {   
        ...
        if (first < second)
        {
            ans += "(";
            ans.push_back(root);
            ans += getSmallestExpression(parent,first) + getSmallestExpression(parent,second) + ")";
        }
        else
        {
            ans += "(";
            ans.push_back(root);
            ans += getSmallestExpression(parent,second) + getSmallestExpression(parent,first) + ")";
        }
    }
    else
    {
        ...
        ans += "(";
        ans.push_back(root);
        ans += getSmallestExpression(parent,first) + ")";
    }
    return ans;
}
...
int main() {
    ...
    if(parseInput(input,parentChild)) {
    ...
    else if(checkCycle(parent,uniqueVertices)) {
        cout<<"E4"<<endl;
        return 0;
    }
        ...
        // cout<<getSmallestExpression(parent,root)<<endl;
        return 0;
    }
